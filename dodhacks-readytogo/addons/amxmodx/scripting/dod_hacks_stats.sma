...
#if !defined MAX_BODYHITS
#define      MAX_BODYHITS     8
#endif

#if !defined MaxClients
new          MaxClients;
#define      WRAPPED_FOR_OLDER_AMXX
#endif

#if !defined DODX_MAX_STATS
enum {
	DODX_KILLS = false,
	DODX_DEATHS,
	DODX_HEADSHOTS,
...
public plugin_init() {
   ...
   * ---------
   */
  register_plugin("DoD Hacks: Stats", AMXX_VERSION_STR, "AMXX Dev Team");
  if (safeDisablePluginIfRunning("DoD.Hacks.Stats.LOG", "stats.amxx"))
  {
    new Map[64];
  ...
  register_dictionary("dod_hacks_stats.txt");
  #if defined WRAPPED_FOR_OLDER_AMXX
...
public round_end(){
   ...
   * ---------
   */
  static buf1[384], buf2[384], buf3[384], scoreStr[24], killsStr[24], hitsStr[24], dmgStr[24], hsStr[24];
  new len1, len2, len3, players[MAX_PLAYERS], pnum, stats[DODX_MAX_STATS], bodyhits[MAX_BODYHITS];
  /**
   * ---------
   */
  ...
  for (new i=0;i<pnum;i++) {
      ...
      * ---------
      */
     addCommasInt(score, scoreStr, charsmax(scoreStr));
     addCommasInt(kills, killsStr, charsmax(killsStr));
     addCommasInt(hits, hitsStr, charsmax(hitsStr));
     addCommasInt(damage, dmgStr, charsmax(dmgStr));
     addCommasInt(hs, hsStr, charsmax(hsStr));
     len1 = formatex(buf1, charsmax(buf1),
       "%L: %s^n%s %L",
       players[i], "BEST_SCORE", name1, scoreStr, players[i], "POINTS");
     len2 = formatex(buf2, charsmax(buf2),
       "%L: %s^n%s %L / %s %L",
       players[i], "MOST_KILLS", name2, killsStr, players[i], (kills == 1) ? "KILL" : "KILLS", hsStr, players[i], (hs == 1) ? "HEADSHOT" : "HEADSHOTS");
     len3 = formatex(buf3, charsmax(buf3),
       "%L: %s^n%s %L / %s %L",
       players[i], "MOST_DAMAGE", name3, dmgStr, players[i], "DAMAGE", hitsStr, players[i], (hits == 1) ? "HIT" : "HITS");

     message_begin(MSG_ONE_UNRELIABLE, SVC_DIRECTOR, { 0, 0, 0 }, players[i]);
     write_byte(len1 + 31);
     fillAndEndDirectorMsg(buf1, 100, 200, 0, 0.02, 0.40, 8.0 /** Hold time (sec.). */);

     message_begin(MSG_ONE_UNRELIABLE, SVC_DIRECTOR, { 0, 0, 0 }, players[i]);
     write_byte(len2 + 31);
     fillAndEndDirectorMsg(buf2, 100, 200, 0, 0.02, 0.46, 8.0 /** Hold time (sec.). */);

     message_begin(MSG_ONE_UNRELIABLE, SVC_DIRECTOR, { 0, 0, 0 }, players[i]);
     write_byte(len3 + 31);
     fillAndEndDirectorMsg(buf3, 100, 200, 0, 0.02, 0.52, 8.0 /** Hold time (sec.). */);
     /**
      * ---------
      */
  }
 ...
 * ---------
 */
  static cvDoDPause, bool: bOnce;
  if (!bOnce)
  {
    bOnce = true;
    cvDoDPause = get_cvar_pointer("dodstats_pause");
  }
  if (cvDoDPause && get_pcvar_num(cvDoDPause))
/**
 * ---------
 */
    return 0;
 ...
 * ---------
 */
fillAndEndDirectorMsg
(const Msg[], R, G, B, Float: horPos, Float: verPos, Float: holdTime)
{
    write_byte(DRC_CMD_MESSAGE);
    write_byte(2 /** Effect type. */); /// 0 = none, 1 = glitter & 2 = text out.
    write_long(  /** Color.       */
         B /** Blue.  */        +
        (G /** Green. */ <<  8) +
        (R /** Red.   */ << 16)
    );
    write_long(unsigned: horPos   /** Horizontal position. */);
    write_long(unsigned: verPos   /** Vertical position */   );
    write_long(unsigned: 0.01     /** Fade in time. */       );
    write_long(unsigned: 0.7      /** Fade out time. */      );
    write_long(unsigned: holdTime /** Hold time. */          );
